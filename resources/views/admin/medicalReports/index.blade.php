...
<div class="card">
    ...
    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-MedicalReport">
            <thead>
                <tr>
                    <th width="10">
                    ...
                    <td>
                        <select class="search">
                            ...
                            @foreach($appointments as $key => $item)
                                <option value="{{ $item->appointment_date }}">{{ $item->appointment_date }}</option>
                            @endforeach
                        </select>
                    </td>
                    <td>
                        <select class="search" strict="true">
                            <option value>{{ trans('global.all') }}</option>
                            @foreach(App\Models\MedicalReport::REPORT_TYPE_SELECT as $key => $item)
...
@endcan
  ...
  let dtOverrideGlobals = {
    ...
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.medical-reports.index') }}",
    columns: [
      { data: 'placeholder', name: 'placeholder' },
{ data: 'id', name: 'id' },
{ data: 'appointment_appointment_date', name: 'appointment.appointment_date' },
...
{ data: 'actions', name: '{{ trans('global.actions') }}' }
  ...
  let table = $('.datatable-MedicalReport').DataTable(dtOverrideGlobals);
